...
public class BoardDAO {
	...
	public void insertArticle(BoardVO article) {
		...
		try {
			...
			//새글이기에 다들어가야됨
			strQuery = "insert into board(num, writer, subject, readcount, ref, regdate, content)" 
	...
	public List<BoardVO> getArticles(int start, int end){
		...
		try {
			...
			+ "regdate, readcount, ref, content from ("
			...
			rs = pstmt.executeQuery();
 			...
 			if(rs.next()) {
 				...
 				do {
 					...
 					article.setRegdate(rs.getDate("readcount"));
 					article.setReadcount(rs.getInt("ref"));
 					article.setRef(rs.getInt("regdate"));
	 ...
	 public int updateArticle(BoardVO article) {
			...
			try {
				...
				rs = pstmt.executeQuery();
						...
						strQuery = "update board set writer=?, subject=?, content=? where num=?";
	 ...
	 //검색한 내용을 리스트로 받아옴 (what:검색조건 content:검색내용, 시작번호, 끝번호 ) 시작번호와 끝번호는 페이지 처리용 
	 	public List<BoardVO> getArticles(String what, String content, int start, int end){
			...
			try {
				...
				+ "regdate, readcount, ref, content from ("
				+ "select * from board where "+what+" like '%" +content+"%' order by ref desc asc))"
